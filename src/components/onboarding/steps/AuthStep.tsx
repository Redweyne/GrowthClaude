...
export function AuthStep({ onNext, onBack }: AuthStepProps) {
  ...
  const mapError = useCallback((raw: string): string => {
    ...
    if (lower.includes('supabase is not configured')) {
      return t('onboarding.auth.notConfigured' as any);
    }
  ...
  return (
    <div className="min-h-[70dvh] flex flex-col relative z-20 pointer-events-auto">
        ...
        {phase === 'form' && (
          ...
          >
            ...
            <motion.div
              ...
              className={isRTL ? 'self-end' : 'self-start'}
            >
              <button
                type="button"
                data-testid="auth-back-btn"
                onClick={onBack}
                className={`flex items-center gap-1 text-stone-500 light:text-stone-600 hover:text-stone-300 light:hover:text-stone-900 transition-colors mb-8 group active:scale-95 touch-manipulation ${isRTL ? 'flex-row-reverse' : ''}`}
              >
                {isRTL ? (
            ...
            >
              <button
                ...
                className="w-full py-4 rounded-xl border border-amber-500/40 bg-amber-500/10 hover:bg-amber-500/18 text-amber-200 light:text-amber-900 font-medium transition-all flex items-center justify-center gap-3 mb-5 disabled:opacity-40 disabled:cursor-not-allowed active:scale-[0.98] touch-manipulation"
                style={{
                  boxShadow: !googleRedirecting ? '0 0 18px rgba(251,191,36,0.12)' : 'none',
                }}
              >
                {googleRedirecting ? (
                  <motion.div
            ...
            >
              <button
                ...
                className="w-full mt-4 py-4 rounded-xl bg-gradient-to-r from-amber-600 to-amber-500 text-stone-950 font-semibold text-base transition-all disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2 active:scale-[0.98] touch-manipulation"
                style={{
                  boxShadow: 'none',
                }}
              >
            ...
            >
              <button
                type="button"
                data-testid="auth-wanderer-btn"
                onClick={handleWanderer}
                className="mt-4 py-2 text-xs text-stone-600 light:text-stone-500 hover:text-stone-400 light:hover:text-stone-700 transition-colors underline underline-offset-2 decoration-stone-700 hover:decoration-stone-500 light:decoration-stone-400 active:scale-95 touch-manipulation"
              >
