...
import { AgoraPostDetail } from './AgoraPostDetail';
...
export function AgoraView({ onClose }: AgoraViewProps) {
  ...
  // Keep selected post in sync with store (vote counts change optimistically)
        ...
        <div className="px-4 py-4 space-y-3 pb-24">
          <AnimatePresence mode="popLayout">
            {isLoading && posts.length === 0 ? (
              ...
              >
                <Landmark size={40} className="text-stone-700 mb-4" />
                <p className="text-stone-400 light:text-stone-600 font-medium mb-1">{t('agora.emptyTitle')}</p>
                <p className="text-stone-600 text-sm max-w-xs">{t('agora.emptyDescription')}</p>
              </motion.div>
            ) : (
              posts.map((post) => (
                <AgoraPostCard
                  ...
                  onClick={() => { setSelectedPost(post); hapticLight(); }}
      ...
      <AnimatePresence>
        {currentSelectedPost && (
          <AgoraPostDetail
            ...
            hasVoted={userVoteIds.includes(currentSelectedPost.id)}
            onVote={() => handleVote(currentSelectedPost.id)}
